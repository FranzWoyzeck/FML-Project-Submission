...
def get_permutation_value(coin_relative, position):
    if position == (1, 1):
        #left top
        return 1
        return np.array([3, 0, 1, 2, 4])
    elif position == (1, 15):
        #left bottem
        return 0
        ...
        #right bottem
        return 3
        return np.array([1, 2, 3, 0, 4])
    elif position == (15, 1):
        #right top
        return 2
        return np.array([2, 3, 0, 1, 4])
    else:
        if position[0] == 1:
            if coin_relative[0] == 0:
                if coin_relative[1] < 0:
                    return 0
                    return np.array([0, 1, 2, 3, 4])
                else:
                    return 1
        ...
        if coin_relative[0] >= 0 >= coin_relative[1]:
            # left bottem
            return 0
            return np.array([0, 1, 2, 3, 4])
        elif coin_relative[1] >= 0 >= coin_relative[0]:
            # right top
            return 2
            return np.array([2, 3, 0, 1, 4])
        elif coin_relative[1] >= 0 and coin_relative[0] >= 0:
            # left top
            return 1
            return np.array([3, 0, 1, 2, 4])
        elif coin_relative[1] <= 0 and coin_relative[0] <= 0:
...
def get_position_state_relative(state, perm):
...
def get_relative_coin_state(coin, perm):
    if perm == 1 or perm == 3:
        coin = coin[[1, 0]]
    if coin[1] == 0 and coin[0] != 0:
        return 0
...
def get_discrete_state(state):
    ...
    index_permutation = get_permutation_value(coin, state.get('self')[3])
    index_space = get_position_state_relative(state, index_permutation)
    index_coin = get_relative_coin_state(coin, index_permutation)
    #print(index_space, index_coin, index_permutation, coin)
    return (index_space, index_coin), PERMUTATION[index_permutation]
...
def setup(self):
    ...
    if self.train or not os.path.isfile("my-saved-model.pt"):
        ...
        if not os.path.isfile("my-saved-model.pt"):
            ...
            q_table = np.full(([3] + [3] + [len(ACTIONS)]), -1.5)
            self.model = q_table
        else:
            self.epsilon_decay_value = self.epsilon / (self.end_epsilon_decaying - self.start_epsilon_decaying)
...
def act(self, game_state: dict) -> str:
    ...
    indices, permutation = get_discrete_state(game_state)
    action = ACTIONS[permutation]
    return action[np.argmax(self.model[indices])]
