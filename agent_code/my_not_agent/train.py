...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state:
        if True:
            ...
            vec_alt = np.abs(
                np.array(old_game_state.get('coins'))[CLOSEST_COIN] - np.array(old_game_state.get('self')[3]))
            vec_neu = np.abs(
                np.array(new_game_state.get('coins'))[CLOSEST_COIN] - np.array(new_game_state.get('self')[3]))
            ...
            discrete_state, permutation = get_discrete_state(old_game_state)

            current_q = self.model[discrete_state]
            new_discrete_state, permutation = get_discrete_state(new_game_state)
            max_future_q = np.max(self.model[new_discrete_state])
            action = int(np.where(np.array(ACTIONS) == self_action)[0])

            reward = reward_from_events(self, events)

            new_model = (1 - LEARNING_RATE) * current_q[action] + LEARNING_RATE * (reward + DISCOUNT
                                                                 * max_future_q)
    ...
    self.transitions.append(Transition(old_game_state, self_action, state_to_features(new_game_state), reward))
